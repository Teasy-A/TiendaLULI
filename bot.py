...
CONFIG = {
    ...
    "BOT_TOKEN": "PON_AQUI_TU_TOKEN_DE_BOTFATHER",

    # Tu ID de Telegram personal (el admin que recibe los avisos de pedidos)
    # Para obtenerlo: escribele a @userinfobot en Telegram, te dice tu ID
    "ADMIN_CHAT_ID": "PON_AQUI_TU_ID_DE_TELEGRAM",

    # URL pública donde alojaste index.html (la Mini App)
    "MINI_APP_URL": "https://TU-DOMINIO-AQUI.com/index.html",
...
# Estructura simple en memoria — para 1-20 productos y bajo volumen es
# suficiente. Si más adelante el volumen crece, esto se reemplaza por
# una base de datos real (ej. SQLite) para no perder pedidos si el bot
# se reinicia.
pending_orders = {}
...
async def on_web_app_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Guardamos el pedido como "pendiente de confirmar en blockchain"
    ...
    # Avisamos al admin INMEDIATAMENTE, aclarando que falta la confirmación
    # on-chain — así no se hace el envío antes de que el pago esté confirmado.
    msg = (
    ...
    # Confirmamos al cliente que su pedido fue recibido
    await update.message.reply_text(
...
# =====================================================================
# Verificación en blockchain — revisa transacciones entrantes a tu wallet
# =====================================================================
def get_incoming_transactions(wallet_address: str, limit: int = 20):
    """
    Consulta las transacciones más recientes recibidas en tu wallet
    usando la API pública de TonAPI (gratis, sin necesidad de API key
...
def extract_comment_and_amount(tx: dict):
    """
    De una transacción de TonAPI, extrae el comentario (memo) de texto
    y el monto recibido en nanoTON, si existen.
    """
    ...
    if not comment:
        # Algunos formatos lo devuelven directo en msg_data
...
async def check_blockchain_for_payments(context: ContextTypes.DEFAULT_TYPE):
    """
    Tarea periódica: revisa las transacciones entrantes a tu wallet y
    las compara contra los pedidos pendientes por su orderId (memo).
    """
    ...
    for tx in transactions:
        ...
        # Tolerancia del 2% para variaciones de gas/redondeo
...
def main():
    ...
    # Tarea periódica que revisa la blockchain
